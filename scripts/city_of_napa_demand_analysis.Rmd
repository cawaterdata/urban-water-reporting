...
```{r}
all_reports_demand <- bind_rows(uwmp_demand_table, wlr_demand_table, cr_demand_table, ear_demand_table) %>% glimpse
View(all_reports_demand)
```

#### Stacked Bar Plot: 
```{r}
ggplot(all_reports_demand, aes(y = report_name, x = volume_af, fill = use_type)) +
  geom_col() + 
  labs(x = "Reported Annual AF Demand", y = "", 
       title = "Reported Annual Water Demand Across Reporting Requirements") +
  theme_minimal() +
