...
"Santa Fe Irrigarion District"

Load in data: 
```{r, warning=FALSE, message=FALSE}
supply_and_demand_data <- read_rds("../data/supply_and_demand_data.rds") %>% 
  filter(supplier_name  == "Santa Fe Irrigation District") %>% glimpse
```

```{r}
agency_of_interest <- "Santa Fe Irrigarion District"
```
...
ear_demand_table <- supply_and_demand_data %>% 
  ...
  mutate(report_name = "EAR", 
         supplier_name = "Santa Fe Irrigarion District", 
         year = 2020, 
         category = "demand")
...
demand_by_report <- tibble(
  "Agency" = rep("Santa Fe Irrigarion District", 4),
...
demand_by_report
write_rds(demand_by_report, "../data/santa_fe_demand_af_volumes.rds")
```
...
total_water_demand_table <- 
  tibble("agency" = rep(c("Santa Fe Irrigarion District"), 6),
...
santa_fe_d_with_subcategories <- write_rds(all_reports_demand, "../data/santa_fe_d_with_subcategories.rds")
...
- Appear to be similar for UWMP appears to be smaller than WLR, none for CR

*Residential*
- Larger for CR than UWMP 

*Recycled*
- Looks similar between UWMP and CR

*Losses*
- Only reported by WLR

*Landscape*
- Only reported by UWMP

*Commercial industrial institutional*
- CR is much larger than UWMP 

*Agricultural irrigation*
- CR is larger than UWMP 

**Insights**
- CR larger because higher residential and commercial usage reported 
